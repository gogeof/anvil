...
const AUTO_COMPACTION_THRESHOLD_ENV_VAR: &str = "ANVIL_AUTO_COMPACT_INPUT_TOKENS";
    ...
    T: ToolExecutor,
{
    ...
    #[must_use]
    pub fn with_max_budget_usd(mut self, max_budget_usd: Option<f64>) -> Self {
        ...
        loop {
            ...
            if let Some(usage) = usage {
                ...
                if let Some(max_budget) = self.max_budget_usd {
                    let model = self.api_client.model_name();
                    let cumulative = self.usage_tracker.cumulative_usage();
                    let cost = cumulative.estimate_cost_usd_for_model(model);
                    if cost > max_budget {
...
mod tests {
    ...
    #[test]
    fn run_turn_errors_when_budget_is_exceeded() {
        struct ExpensiveApi;
        ...
        // given — budget of $1, but one turn costs ~$2.60 (500K*2 + 200K*8 per million)
        let mut runtime = ConversationRuntime::new(
            Session::new(),
            ExpensiveApi,
            StaticToolExecutor::new(),
        ...
        // when
        let error = runtime
            .run_turn("expensive", None)
            .expect_err("budget exceeded should stop the conversation");
    ...
    #[test]
    fn run_turn_succeeds_when_budget_not_exceeded() {
        struct CheapApi;
        ...
        impl ApiClient for CheapApi {
            fn stream(
                ...
                Ok(vec![
                    AssistantEvent::TextDelta("cheap response".to_string()),
                    AssistantEvent::Usage(TokenUsage {
                        input_tokens: 100,
                        output_tokens: 50,
                        cache_creation_input_tokens: 0,
        ...
        // given — budget of $100, but this turn costs < $0.01
        let mut runtime = ConversationRuntime::new(
            Session::new(),
            CheapApi,
            StaticToolExecutor::new(),
        ...
        .with_max_budget_usd(Some(100.0));

        // when
        let summary = runtime
            .run_turn("cheap", None)
            .expect("budget not exceeded should succeed");
