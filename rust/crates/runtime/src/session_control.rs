...
pub struct SessionStore {
    ...
    /// `/home/user/project/.anvil/sessions/a1b2c3d4e5f60718/`.
...
impl SessionStore {
    ...
    ///
    /// The on-disk layout becomes `<cwd>/.anvil/sessions/<workspace_hash>/`.
...
fn format_missing_session_reference(reference: &str, sessions_root: &Path) -> String {
    // #80: show the actual workspace-fingerprint directory instead of lying about .anvil/sessions/
    let fingerprint_dir = sessions_root
        .file_name()
        .and_then(|f| f.to_str())
        .unwrap_or("<unknown>");
    format!(
        "session not found: {reference}\nHint: managed sessions live in .anvil/sessions/{fingerprint_dir}/ (workspace-specific partition).\nTry `{LATEST_SESSION_REFERENCE}` for the most recent session or `/session list` in the REPL."
    )
}
...
mod tests {
    ...
    fn session_store_rejects_legacy_session_from_other_workspace() {
        ...
        let store_b = SessionStore::from_cwd(&workspace_b).expect("store b should build");
        let legacy_root = workspace_b.join(".anvil").join("sessions");
    ...
    fn session_store_loads_safe_legacy_session_from_same_workspace() {
        ...
        let store = SessionStore::from_cwd(&base).expect("store should build");
        let legacy_root = base.join(".anvil").join("sessions");
