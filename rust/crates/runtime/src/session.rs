...
impl Session {
    ...
    ///
    /// A "turn" starts with a user message. This removes everything from the
    /// Nth-to-last user message to the end, including any assistant responses
    ...
    pub fn rewind(&mut self, turns: usize) -> usize {
        ...
        // Find the positions of all user messages
        ...
        // If we don't have enough turns, rewind as much as possible
        let turn_index = if turns >= user_positions.len() {
            0
        } else {
            user_positions.len() - turns
        };

        // Get the starting position for removal
        let start_pos = user_positions.get(turn_index).copied().unwrap_or(0);

        // Calculate how many messages we'll remove
        let removed_count = self.messages.len() - start_pos;

        if removed_count == 0 {
            return 0;
        }

        // Truncate the messages vector
        self.messages.truncate(start_pos);
        self.touch();
...
mod tests {
    ...
    #[test]
    fn rewind_removes_last_user_message_and_everything_after() {
        // given
        let mut session = Session::new();
        session.push_user_text("first").expect("should push");
        session
            .push_message(ConversationMessage::assistant(vec![ContentBlock::Text {
                text: "response 1".to_string(),
            }]))
            .expect("should push");
        session.push_user_text("second").expect("should push");
        session
            .push_message(ConversationMessage::assistant(vec![ContentBlock::Text {
        ...
        assert_eq!(session.messages[0], ConversationMessage::user_text("first"));
    }

    #[test]
    fn rewind_multiple_turns_removes_correct_messages() {
        // given
        let mut session = Session::new();
        session.push_user_text("turn 1").expect("should push");
        session
            .push_message(ConversationMessage::assistant(vec![ContentBlock::Text {
                text: "response 1".to_string(),
            }]))
            .expect("should push");
        session.push_user_text("turn 2").expect("should push");
        session
            .push_message(ConversationMessage::assistant(vec![ContentBlock::Text {
                text: "response 2".to_string(),
            }]))
            .expect("should push");
        session.push_user_text("turn 3").expect("should push");
        session
            .push_message(ConversationMessage::assistant(vec![ContentBlock::Text {
        ...
        assert_eq!(
            session.messages[0],
            ConversationMessage::user_text("turn 1")
        );
    }

    #[test]
    fn rewind_zero_does_nothing() {
        // given
        let mut session = Session::new();
        session.push_user_text("hello").expect("should push");

        // when
        let removed = session.rewind(0);
    ...
    #[test]
    fn rewind_more_than_available_clears_all() {
        // given
        let mut session = Session::new();
        session.push_user_text("only message").expect("should push");
        session
            .push_message(ConversationMessage::assistant(vec![ContentBlock::Text {
