...
pub fn pricing_for_model(model: &str) -> Option<ModelPricing> {
    ...
    // Legacy DeepSeek model names
    if normalized.contains("deepseek") {
        return Some(ModelPricing {
            input_cost_per_million: 0.50,
            output_cost_per_million: 2.0,
            cache_creation_cost_per_million: 0.50,
...
impl TokenUsage {
    ...
    /// Estimate cost using pricing for the given model name.
    /// Falls back to default Sonnet-tier pricing when the model is unknown.
    #[must_use]
    pub fn estimate_cost_usd_for_model(self, model: &str) -> f64 {
...
mod tests {
    ...
    #[test]
    fn estimate_cost_usd_for_model_returns_zero_for_no_usage() {
        let usage = TokenUsage {
            input_tokens: 0,
            output_tokens: 0,
            cache_creation_input_tokens: 0,
        ...
        let cost = usage.estimate_cost_usd_for_model("deepseek-v4-pro");
        assert_eq!(cost, 0.0);
    }

    #[test]
    fn estimate_cost_usd_for_model_uses_deepseek_pricing() {
        // 1M input + 500K output at DeepSeek V4 Pro pricing
        // input: 1_000_000 * $2 / 1_000_000 = $2.00
        // output: 500_000 * $8 / 1_000_000 = $4.00
        // total: $6.00
        let usage = TokenUsage {
            input_tokens: 1_000_000,
            output_tokens: 500_000,
    ...
    #[test]
    fn estimate_cost_usd_for_model_falls_back_to_default_for_unknown() {
        let usage = TokenUsage {
            input_tokens: 1_000_000,
            output_tokens: 500_000,
            cache_creation_input_tokens: 0,
        ...
        // Unknown model should use default Sonnet-tier pricing
        let cost = usage.estimate_cost_usd_for_model("unknown-model");
        assert!(cost > 0.0);
        // Sonnet: 1M*$15 + 500K*$75 = $15 + $37.5 = $52.5
        assert!((cost - 52.5).abs() < 0.01, "expected ~$52.5 got ${}", cost);
    }
}
