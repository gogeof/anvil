...
pub const FRONTIER_MODEL_NAME: &str = "Anvil";
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "Anvil instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# Instruction files".to_string()];
...
mod tests {
    ...
    fn load_system_prompt_reads_claude_files_and_config() {
        ...
        let original_claw_home = std::env::var("ANVIL_CONFIG_HOME").ok();
...
",
    ...
    fn renders_generic_model_family_identity_without_claude_label() {
        ...
        // then: the model family line is neutral and excludes Anvil
        assert_eq!(model_family_line, " - Model family: an AI assistant");
        assert!(!model_family_line.contains("Anvil"));
    }
    ...
    fn discovers_dot_claude_instructions_markdown() {
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".anvil/instructions.md")));
        assert!(
