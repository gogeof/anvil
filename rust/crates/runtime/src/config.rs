...
impl ConfigLoader {
    ...
    pub fn load(&self) -> Result<RuntimeConfig, ConfigError> {
        ...
        // #env-inject: apply env vars from config to process environment.
        // This lets users store API keys (e.g. DEEPSEEK_API_KEY) in
        // settings.json's "env" object rather than in shell profiles.
        if let Some(env_map) = optional_string_map(&merged, "env", "merged settings").ok().flatten() {
...
fn read_optional_json_object(path: &Path) -> Result<Option<ParsedConfigFile>, ConfigError> {
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".anvil.json");
...
mod tests {
    use super::{
        ...
        RuntimePluginConfig, ANVIL_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn rejects_invalid_hook_entries_before_merge() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".anvil");
        let project_settings = cwd.join(".anvil").join("settings.json");
        fs::create_dir_all(cwd.join(".anvil")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
