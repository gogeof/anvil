...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "plan",
        aliases: &[],
        summary: "Create a structured implementation plan for a task",
...
pub fn validate_slash_command_input(
    ...
    Ok(Some(match command {
        ...
        "plan" => SlashCommand::Plan { task: remainder },
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    if let Ok(claw_config_home) = env::var("ANVIL_CONFIG_HOME") {
        push_unique_root(
            &mut roots,
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    ...
    if let Some(home) = env::var_os("HOME") {
        return Ok(PathBuf::from(home).join(".anvil").join("skills"));
    }
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "unable to resolve a skills install root; set ANVIL_CONFIG_HOME or HOME",
    ))
}
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Sources          .anvil/agents, ~/.anvil/agents, $ANVIL_CONFIG_HOME/agents".to_string(),
    ];
...
fn render_agents_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "sources": [".anvil/agents", "~/.anvil/agents", "$ANVIL_CONFIG_HOME/agents"],
        },
...
fn render_skills_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "install_root": "$ANVIL_CONFIG_HOME/skills or ~/.anvil/skills",
            "sources": [
                ".anvil/skills",
                ".omc/skills",
                ".agents/skills",
...
fn render_mcp_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Sources          .anvil/settings.json, .anvil/settings.local.json".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_mcp_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            "slash_command": "/mcp [list|show <server>|help]",
            "direct_cli": "claw mcp [list|show <server>|help]",
            "sources": [".anvil/settings.json", ".anvil/settings.local.json"],
        },
        "unexpected": unexpected,
    })
}
...
mod tests {
    ...
    fn resolves_project_skills_and_legacy_commands_from_shared_registry() {
        ...
        let project_skills = workspace.join(".anvil").join("skills");
        let legacy_commands = workspace.join(".anvil").join("commands");
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help
            .contains("Sources          .anvil/agents, ~/.anvil/agents, $ANVIL_CONFIG_HOME/agents"));
    ...
    fn renders_mcp_reports_from_loaded_config() {
        ...
        fs::create_dir_all(workspace.join(".anvil")).expect("workspace config dir");
        fs::create_dir_all(&config_home).expect("config home");
        fs::write(
            workspace.join(".anvil").join("settings.json"),
            r#"{
              "mcpServers": {
                "alpha": {
                  "command": "uvx",
        ...
        fs::write(
            workspace.join(".anvil").join("settings.local.json"),
            r#"{
              "mcpServers": {
                "remote": {
                  "type": "ws",
    ...
    fn renders_mcp_reports_as_json() {
        ...
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["sources"][0], ".anvil/settings.json");
    ...
    fn mcp_degrades_gracefully_on_malformed_mcp_config_144() {
        ...
        fs::create_dir_all(workspace.join(".anvil")).expect("create workspace .anvil dir");
        fs::create_dir_all(&config_home).expect("create config home");
        // One valid server + one malformed entry missing `command`.
        fs::write(
            workspace.join(".anvil.json"),
            r#"{
  "mcpServers": {
...
"#,
        ...
        .expect("write malformed .anvil.json");
