...
fn resumed_config_command_loads_settings_files_end_to_end() {
    ...
    fs::create_dir_all(project_dir.join(".anvil")).expect("project config dir should exist");
    ...
    fs::write(
        project_dir.join(".anvil").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    let output = run_claw_with_env(
        ...
        &[("ANVIL_CONFIG_HOME", config_home.to_str().expect("utf8 path"))],
    );
...
fn resumed_status_command_emits_structured_json_when_requested() {
    ...
    // when
    // Use an isolated ANVIL_CONFIG_HOME so ~/.anvil/settings.json is not loaded,
...
#[test]
fn resumed_context_command_emits_structured_json() {
    let temp_dir = unique_temp_dir("resume-context-json");
    fs::create_dir_all(&temp_dir).expect("temp dir should exist");
    let session_path = temp_dir.join("session.jsonl");
    let mut session = workspace_session(&temp_dir);
    session
        .push_user_text("context json fixture")
        .expect("write ok");
    session.save_to_path(&session_path).expect("persist ok");
    ...
    let stdout = String::from_utf8(output.stdout).expect("utf8");
    let parsed: Value = serde_json::from_str(stdout.trim()).expect("should be json");
    assert_eq!(parsed["kind"], "context");
    assert_eq!(parsed["message_count"], 1);
    assert!(parsed["input_tokens"].is_number());
    assert!(parsed["output_tokens"].is_number());
    assert!(parsed["total_tokens"].is_number());
    assert!(parsed["context_window_tokens"].is_number());
}
...
fn run_claw_with_env(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_anvil"));
