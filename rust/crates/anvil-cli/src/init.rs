...
const STARTER_ANVIL_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_ENTRIES: [&str; 3] = [".anvil/settings.local.json", ".anvil/sessions/", ".anvilhip/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let anvil_dir = cwd.join(".anvil");
    artifacts.push(InitArtifact {
        name: ".anvil/",
        status: ensure_dir(&anvil_dir)?,
    });

    let anvil_json = cwd.join(".anvil.json");
    artifacts.push(InitArtifact {
        name: ".anvil.json",
        status: write_file_if_missing(&anvil_json, STARTER_ANVIL_JSON)?,
    });
...
mod tests {
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".anvil/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".anvil/sessions/").count(), 1);
        assert_eq!(gitignore.matches(".anvilhip/").count(), 1);
