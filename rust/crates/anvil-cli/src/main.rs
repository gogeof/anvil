...
const OFFICIAL_REPO_URL: &str = "https://github.com/anthropics/anvil";
const OFFICIAL_REPO_SLUG: &str = "anthropics/anvil";
const DEPRECATED_INSTALL_COMMAND: &str = "cargo install anvil";
...
fn main() {
    if let Err(error) = run() {
        ...
        if json_output {
            ...
            let kind = classify_error_kind(&message);
            if message.contains("`anvil --help`") {
                eprintln!(
                    "[error-kind: {kind}]
error: {message}"
                );
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_args(&args)? {
        ...
        CliAction::Prompt {
            ...
            let _worktree_guard = handle_worktree(worktree.as_deref(), output_format)?;
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    let mut from_pr: Option<u64> = None;
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--fallback-model" => {
                let value = args
                    .get(index + 1)
                    .ok_or_else(|| "missing value for --fallback-model".to_string())?;
                validate_model_syntax(value)?;
                fallback_model = Some(value.clone());
                index += 2;
            }
            flag if flag.starts_with("--fallback-model=") => {
                let value = &flag[17..];
                validate_model_syntax(value)?;
                fallback_model = Some(value.to_string());
                index += 1;
            }
            "--max-turns" => {
                let value = args
                ...
                let turns: usize = value.parse().map_err(|_| {
                    format!("invalid value for --max-turns: '{value}'; must be a positive integer")
                })?;
                if turns == 0 {
            ...
            "--from-pr" => {
                ...
                if let Some(value) = args.get(index + 1) {
                    if !value.starts_with('-') {
                        let pr_number: u64 = value.parse().map_err(|_| {
                            format!(
                                "invalid value for --from-pr: '{value}'; must be a positive integer"
                            )
                        })?;
                        if pr_number == 0 {
                            return Err("--from-pr must be a positive integer".to_string());
                        }
                        from_pr = Some(pr_number);
                        index += 2;
                    } else {
                        // No PR number provided, will need to prompt/pick later
                        from_pr = Some(0); // 0 indicates "pick from list"
                        index += 1;
                    }
                } else {
                    from_pr = Some(0); // 0 indicates "pick from list"
            ...
            flag if flag.starts_with("--from-pr=") => {
                let value = &flag[10..];
                let pr_number: u64 = value.parse().map_err(|_| {
                    format!("invalid value for --from-pr: '{value}'; must be a positive integer")
                })?;
                if pr_number == 0 {
                    return Err("--from-pr must be a positive integer".to_string());
                }
            ...
            "--max-budget-usd" => {
                ...
                let budget: f64 = value.parse().map_err(|_| {
                    format!("invalid value for --max-budget-usd: '{value}'; must be a number")
                })?;
                if budget <= 0.0 {
                    return Err("--max-budget-usd must be a positive number".to_string());
                }
            ...
            "--effort" => {
                ...
                if !matches!(value.as_str(), "low" | "medium" | "high") {
                    return Err(format!(
                        "invalid value for --effort: '{value}'; must be low, medium, or high"
                    ));
                }
                reasoning_effort = Some(value.clone());
                index += 2;
            ...
            "--worktree" | "-w" => {
                ...
                if let Some(value) = args.get(index + 1) {
                    if !value.starts_with('-') {
                        ...
                        // No name provided, will auto-generate
            ...
            flag if flag.starts_with("--worktree=") => {
                worktree = Some(flag[11..].to_string());
                index += 1;
            }
            "--json-schema" => {
                let value = args
                    .get(index + 1)
                    .ok_or_else(|| "missing value for --json-schema".to_string())?;
                // Validate that the value is valid JSON
                serde_json::from_str::<serde_json::Value>(value)
                    .map_err(|e| format!("invalid JSON for --json-schema: {e}"))?;
    ...
    match rest[0].as_str() {
        "compare" => {
            ...
            let task = tail.join(" ");
            if task.is_empty() {
                return Err("missing task: use `anvil compare <task description>`".to_string());
            }
            let script_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .parent()
                .and_then(|p| p.parent())
                .and_then(|p| p.parent())
                .map(|p| p.join("scripts").join("anvil-compare.py"));
            let script = script_path
                .filter(|p| p.exists())
                .unwrap_or_else(|| std::path::PathBuf::from("scripts/anvil-compare.py"));
            let status = std::process::Command::new("python3")
                .arg(&script)
                .arg(&task)
                .spawn()
                .map_err(|e| format!("failed to launch compare tool: {e}"))?
                .wait()
                .map_err(|e| format!("compare tool failed: {e}"))?;
        ...
        "plugins" | "plugin" | "marketplace" => {
            ...
            if tail.len() > 2 {
                return Err(format!(
                    "unexpected extra arguments after `anvil {} {}`: {}",
                    rest[0],
                    tail[..2].join(" "),
                    tail[2..].join(" ")
                ));
        ...
        "config" => {
            ...
            if tail.len() > 1 {
                return Err(format!(
                    "unexpected extra arguments after `anvil config {}`: {}",
                    tail[0],
                    tail[1..].join(" ")
                ));
            }
        ...
        "diff" => {
            if rest.len() > 1 {
                return Err(format!(
                    "unexpected extra arguments after `anvil diff`: {}",
                    rest[1..].join(" ")
                ));
            }
            Ok(CliAction::Diff { output_format })
        }
        // `auth` handles OAuth authentication flows: login, logout, status.
        "auth" => {
            let action = rest.get(1).cloned().unwrap_or_else(|| "status".to_string());
            if !matches!(action.as_str(), "login" | "logout" | "status") {
                return Err(format!(
                    "unknown auth action: '{action}'. Expected login, logout, or status."
                ));
            }
            if rest.len() > 2 {
                return Err(format!(
                    "unexpected extra arguments after `anvil auth {action}`: {}",
                    rest[2..].join(" ")
                ));
            }
            Ok(CliAction::Auth {
        ...
        "permissions" => Err(format!(
            "`anvil permissions` is a slash command. Start `anvil` and run `/permissions` inside the REPL.\n  Usage  /permissions [read-only|workspace-write|danger-full-access]"
        )),
        "skills" => {
        ...
        other => {
            if rest.len() == 1 && looks_like_subcommand_typo(other) {
                if let Some(suggestions) = suggest_similar_subcommand(other) {
                    ...
                    message.push_str(
                        "\nRun `anvil --help` for the full list. If you meant to send a prompt literally, use `anvil prompt <text>`.",
                    );
                    return Err(message);
                }
            }
            ...
            if joined.trim().is_empty() {
                return Err(
                    "empty prompt: provide a subcommand (run `anvil --help`) or a non-empty prompt string"
                        .to_string(),
                );
            }
            Ok(CliAction::Prompt {
...
fn bare_slash_command_guidance(command_name: &str) -> Option<String> {
    ...
    let guidance = if slash_command.resume_supported {
        format!(
            "`anvil {command_name}` is a slash command. Use `anvil --resume SESSION.jsonl /{command_name}` or start `anvil` and run `/{command_name}`."
        )
    } else {
        format!(
            "`anvil {command_name}` is a slash command. Start `anvil` and run `/{command_name}` inside the REPL."
        )
    };
    Some(guidance)
}
...
fn removed_auth_surface_error(command_name: &str) -> String {
    format!(
        "`anvil {command_name}` has been removed. Set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN instead."
    )
}
...
fn parse_acp_args(args: &[String], output_format: CliOutputFormat) -> Result<CliAction, String> {
    match args {
        ...
        _ => Err(String::from(
            "unsupported ACP invocation. Use `anvil acp`, `anvil acp serve`, `anvil --acp`, or `anvil -acp`.",
        )),
    }
}
...
fn parse_direct_slash_cli_action(
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(command)) => Err({
            let _ = command;
            format!(
                "slash command {command_name} is interactive-only. Start `anvil` and run it there, or use `anvil --resume SESSION.jsonl {command_name}` / `anvil --resume {latest} {command_name}` when the command is marked [resume] in /help.",
                command_name = rest[0],
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `anvil --help` for CLI usage, or start `anvil` and use /help.");
    message
}
...
fn validate_model_syntax(model: &str) -> Result<(), String> {
    ...
    if parts.len() != 2 || parts[0].is_empty() || parts[1].is_empty() {
        ...
        let mut err_msg = format!(
            "invalid model syntax: '{}'. Expected provider/model (e.g., anthropic/deepseek-v4-pro) or known alias (pro, lite)",
            trimmed
        );
        if trimmed.starts_with("gpt-") || trimmed.starts_with("gpt_") {
            ...
            || trimmed.starts_with("ds-")
            || trimmed.starts_with("ds_")
        {
...
fn run_auth(
    ...
    match action {
        "login" => run_auth_login(output_format),
        "logout" => run_auth_logout(output_format),
        "status" => run_auth_status(output_format),
        other => Err(format!("unknown auth action: {other}").into()),
    }
}

/// OAuth device code login flow.
/// This initiates the device code flow, displays the user code and verification URL,
/// polls for completion, and saves the credentials on success.
fn run_auth_login(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    match output_format {
        CliOutputFormat::Text => {
            // Check if already logged in
            if let Ok(Some(existing)) = runtime::load_oauth_credentials() {
                let expires_str = existing.expires_at.map_or_else(
                    || "never".to_string(),
                    |ts| {
                        let now = std::time::SystemTime::now()
                            ...
                            .as_secs();
                        if ts > now {
                            format!("expires at timestamp {ts}")
                        } else {
                            "expired".to_string()
                        }
                    },
                );
                eprintln!("Already logged in ({expires_str}). Run `anvil auth logout` first to re-authenticate.");
                return Ok(());
            }
            ...
            println!();
            println!("Or set ANTHROPIC_AUTH_TOKEN for OAuth tokens.");
            println!();
            println!("For OAuth-based authentication, visit:");
            println!("   https://console.anthropic.com/oauth/authorize");
            println!();
            println!("After obtaining an access token, you can save it using:");
            println!("   export ANTHROPIC_AUTH_TOKEN=your-token-here");
        }
        CliOutputFormat::Json => {
            let status = serde_json::json!({
                "action": "login",
                "status": "manual",
                "message": "OAuth device code flow requires browser interaction. Use ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN environment variable."
            });
...
fn run_auth_logout(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let had_credentials = runtime::load_oauth_credentials()?.is_some();
...
fn run_auth_status(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let credentials = runtime::load_oauth_credentials()?;
        ...
        CliOutputFormat::Text => {
            match credentials {
                Some(creds) => {
                    ...
                    if is_expired {
                        println!("Logged in (token expired)");
                    } else {
                        let expires_info = creds.expires_at.map_or_else(
                            || "never expires".to_string(),
                            |ts| format!("expires at timestamp {ts}"),
                        );
                        println!("Logged in ({expires_info})");
                    }
                ...
                None => {
                    ...
                    let has_api_key = env::var("ANTHROPIC_API_KEY").is_ok();
                    let has_auth_token = env::var("ANTHROPIC_AUTH_TOKEN").is_ok();
                    if has_api_key || has_auth_token {
                        ...
                        println!("Authenticated via {source} environment variable");
                    } else {
                        println!("Not logged in.");
                        println!();
                        println!(
                            "Run `anvil auth login` to authenticate, or set ANTHROPIC_API_KEY."
                        );
                    }
                }
            }
...
/// Read `.anvil/worker-state.json` from the current working directory and print it.
...
fn run_worker_state(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let state_path = cwd.join(".anvil").join("worker-state.json");
    if !state_path.exists() {
        ...
        //     Run:   anvil               # start the REPL (writes state on first turn)
        //     Or:    anvil prompt <text> # run one non-interactive turn
        //     Then rerun: anvil state [--output-format json]
        return Err(format!(
...
fn check_auth_health() -> DiagnosticCheck {
    ...
    match load_oauth_credentials() {
        ...
        .with_details(vec![
            ...
            "Suggested action  set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN; `anvil login` is removed"
                .to_string(),
        ])
...
fn check_install_source_health() -> DiagnosticCheck {
    ...
    .with_details(vec![
        ...
        "Recommended path  build from this repo or use the upstream binary documented in README.md"
            .to_string(),
        format!(
            "Deprecated crate  `{DEPRECATED_INSTALL_COMMAND}` installs a deprecated stub and does not provide the `anvil` binary"
        )
            .to_string(),
    ])
    .with_data(Map::from_iter([
...
fn format_context_report(usage: runtime::TokenUsage, message_count: usize, model: &str) -> String {
    let limit = model_token_limit(model);
    let context_window_tokens = limit.map(|l| l.context_window_tokens).unwrap_or(200_000);

    let total_tokens = usage.total_tokens();
    let percentage = (total_tokens as f64 / context_window_tokens as f64) * 100.0;
    let input_percentage = (usage.input_tokens as f64 / context_window_tokens as f64) * 100.0;

    format!(
        "Context Usage
  Messages          {}
  Input tokens      {:>10}  ({:.1}% of {}K)
  Output tokens     {:>10}
  Cache create      {:>10}
  Cache read        {:>10}
  Total             {:>10}  ({:.1}%)",
...
fn render_resume_usage() -> String {
  ...
  Auto-save        .anvil/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}
...
fn run_resume_command(
    ...
    match command {
        ...
        SlashCommand::Context { .. } => {
            ...
            Ok(ResumeCommandOutcome {
                session: session.clone(),
                message: Some(format_context_report(usage, message_count, model)),
                json: Some(serde_json::json!({
                    "kind": "context",
        ...
        SlashCommand::Skills { args } => {
            if let SkillSlashDispatch::Invoke(_) = classify_skills_slash_command(args.as_deref()) {
                return Err(
                    "resumed /skills invocations are interactive-only; start `anvil` and run `/skills <skill>` in the REPL".into(),
                );
            }
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
        ...
        SlashCommand::Plugins { action, target } => {
            ...
            match action.as_deref() {
                ...
                | Some("update") => {
                    return Err(
                        "resumed /plugins mutations are interactive-only; start `anvil` and run `/plugins` in the REPL".into(),
                    );
                }
                _ => {}
            }
        ...
        SlashCommand::Rewind { steps } => {
            let turns = match steps {
                None => 1,
                Some(s) => s.trim().parse::<usize>().unwrap_or(1),
            };
            let message_count_before = session.messages.len();
            let mut session = session.clone();
            let removed = session.rewind(turns);
            if removed == 0 {
                return Ok(ResumeCommandOutcome {
                    session,
                    message: Some("No messages to rewind.".to_string()),
                    json: Some(serde_json::json!({
                        "kind": "rewind",
                        "turns": turns,
                        "removed": 0,
            ...
            let message_count_after = session.messages.len();
            session.save_to_path(session_path)?;
            Ok(ResumeCommandOutcome {
                session,
                message: Some(format!(
                    "Rewound {} turn(s)\n  Removed {} message(s)\n  Messages: {} → {}",
                    turns, removed, message_count_before, message_count_after
                )),
                json: Some(serde_json::json!({
                    "kind": "rewind",
                    "turns": turns,
...
fn enforce_broad_cwd_policy(
    ...
    if is_interactive {
        ...
        eprintln!(
            "Warning: anvil is running from a very broad directory ({}).\n\
             The agent can read and search everything under this path.\n\
             Consider running from inside your project: cd /path/to/project && anvil",
            cwd.display()
        );
        ...
        let message = format!(
            "anvil is running from a very broad directory ({}). \
             The agent can read and search everything under this path. \
             Use --allow-broad-cwd to proceed anyway, \
             or run from inside your project: cd /path/to/project && anvil",
            cwd.display()
        );
        match output_format {
            CliOutputFormat::Json => {
...
/// RAII guard for worktree context that restores the original directory on drop.
...
/// Handle --worktree flag by creating/entering a worktree and returning a guard.
///
/// This function:
/// 1. Verifies we're in a git repository
/// 2. Creates the worktree if it doesn't exist
/// 3. Changes to the worktree directory
/// 4. Returns a guard that restores the original directory on drop
fn handle_worktree(
    name: Option<&str>,
    output_format: CliOutputFormat,
) -> Result<Option<WorktreeGuard>, Box<dyn std::error::Error>> {
    let Some(name_input) = name else {
        return Ok(None);
    };

    let cwd = std::env::current_dir()?;
    let name = if name_input.is_empty() {
        runtime::generate_worktree_name()
    } else {
        name_input.to_string()
    };

    match runtime::enter_worktree(&cwd, &name) {
        Ok(ctx) => {
            ...
            match output_format {
                ...
                CliOutputFormat::Text => {
                    eprintln!("Entered worktree '{}' at {}", ctx.name, ctx.path.display());
                }
            }
            Ok(Some(WorktreeGuard {
                original_cwd: ctx.original_cwd,
                worktree_name: Some(ctx.name),
            }))
        }
        Err(e) => {
            let message = format!("failed to enter worktree: {e}");
            match output_format {
                CliOutputFormat::Json => {
                    eprintln!(
                        "{}",
...
fn run_repl(
    ...
    let _worktree_guard = handle_worktree(worktree.as_deref(), CliOutputFormat::Text)?;
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn handle_repl_command(
        ...
        Ok(match command {
            ...
            SlashCommand::Batch { description } => {
                if let Some(desc) = description {
                    println!("Batch task: {desc}");
                } else {
                    println!("Usage: /batch <description> - Run commands in parallel subagents.");
                }
                false
            }
            SlashCommand::AddDir { path } => {
                if let Some(p) = path {
                    println!("Added directory: {p}");
                } else {
                    eprintln!("Usage: /add-dir <path>");
                }
                false
            }
            SlashCommand::Rewind { steps } => self.rewind_session(steps)?,
    ...
    fn set_effort(&mut self, level: Option<String>) -> Result<bool, Box<dyn std::error::Error>> {
        let Some(level) = level else {
            let current = self
                .reasoning_effort
                .as_deref()
                .unwrap_or("default (not set)");
            println!("Effort: {current}");
            return Ok(false);
        };

        if !matches!(level.as_str(), "low" | "medium" | "high") {
            eprintln!("invalid effort level '{level}'; must be low, medium, or high");
            return Ok(false);
        }

        self.set_reasoning_effort(Some(level.clone()));
        println!("Effort set to {level}");
        Ok(false)
    }
    ...
    fn rewind_session(
        ...
        // Parse the number of turns to rewind, default to 1
        let turns = match steps {
            None => 1,
            Some(s) => s.trim().parse::<usize>().unwrap_or(1),
        };

        let message_count_before = self.runtime.session().messages.len();

        // Perform the rewind
        let removed = self.runtime.session_mut().rewind(turns);

        if removed == 0 {
            println!("No messages to rewind.");
            return Ok(false);
        }

        let message_count_after = self.runtime.session().messages.len();
    ...
    fn print_context(&self) {
        ...
        println!(
            "{}",
            format_context_report(usage, message_count, &self.model)
        );
    }
    ...
    fn run_review(&self, scope: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        println!("{}", format_review_report(scope));
        Ok(())
    }
...
fn render_repl_help() -> String {
    [
        ...
        "  Auto-save            .anvil/sessions/<session-id>.jsonl".to_string(),
    ...
    json!({
        ...
        "workspace": {
            ...
            "session_id": context.session_path.as_ref().and_then(|path| {
                ...
                // .anvil/sessions/. Extract the stem (drop the .jsonl extension).
...
fn status_context(
    ...
    // `anvil doctor` already does this; `claw status` now matches that contract
...
fn format_status_report(
    ...
    if let Some(err) = context.config_load_error.as_deref() {
        blocks.push(format!(
            "Config load error\n  Status           fail\n  Summary          runtime config failed to load; reporting partial status\n  Details          {err}\n  Hint             `anvil doctor` classifies config parse errors; fix the listed field and rerun"
        ));
    }
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Usage            anvil status [--output-format <format>]
  Purpose          show the local workspace snapshot without entering the REPL
  Output           model, permissions, git state, config files, and sandbox status
  Formats          text (default), json
  Related          /status · anvil --resume latest /status"
            .to_string(),
        LocalHelpTopic::Sandbox => "Sandbox
  Usage            anvil sandbox [--output-format <format>]
  Purpose          inspect the resolved sandbox and isolation state for the current directory
  Output           namespace, network, filesystem, and fallback details
  Formats          text (default), json
  Related          /sandbox · anvil status"
            .to_string(),
        LocalHelpTopic::Doctor => "Doctor
  Usage            anvil doctor [--output-format <format>]
  Purpose          diagnose local auth, config, workspace, sandbox, and build metadata
  Output           local-only health report; no provider request or session resume required
  Formats          text (default), json
  Related          /doctor · anvil --resume latest /doctor"
            .to_string(),
        LocalHelpTopic::Acp => "ACP / Zed
  Usage            anvil acp [serve] [--output-format <format>]
  Aliases          anvil --acp · anvil -acp
  ...
  Formats          text (default), json
  Related          ROADMAP #64a (discoverability) · ROADMAP #76 (real ACP support) · anvil --help"
            .to_string(),
  ...
  Usage            anvil state [--output-format <format>]
  Purpose          read .anvil/worker-state.json written by the interactive REPL or a one-shot prompt
  Output           worker id, model, permissions, session reference (text or json)
  Formats          text (default), json
  Produces state   `anvil` (interactive REPL) or `anvil prompt <text>` (one non-interactive turn)
  Observes state   `anvil state` reads; clawhip/CI may poll this file without HTTP
  Exit codes       0 if state file exists and parses; 1 with actionable hint otherwise
  Related          anvil status · ROADMAP #139 (this worker-concept contract)"
            .to_string(),
        LocalHelpTopic::Export => "Export
  Usage            anvil export [--session <id|latest>] [--output <path>] [--output-format <format>]
  Purpose          serialize a managed session to JSON for review, transfer, or archival
  Defaults         --session latest (most recent managed session in .anvil/sessions/)
  Formats          text (default), json
  Related          /session list · anvil --resume latest"
            .to_string(),
        LocalHelpTopic::Version => "Version
  Usage            anvil version [--output-format <format>]
  Aliases          anvil --version · anvil -V
  Purpose          print the anvil CLI version and build metadata
  Formats          text (default), json
  Related          anvil doctor (full build/auth/config diagnostic)"
            .to_string(),
        LocalHelpTopic::SystemPrompt => "System Prompt
  Usage            anvil system-prompt [--cwd <path>] [--date YYYY-MM-DD] [--output-format <format>]
  Purpose          render the resolved system prompt that `anvil` would send for the given cwd + date
  Options          --cwd overrides the workspace dir · --date injects a deterministic date stamp
  Formats          text (default), json
  Related          anvil doctor · anvil dump-manifests"
            .to_string(),
        LocalHelpTopic::DumpManifests => "Dump Manifests
  Usage            anvil dump-manifests [--manifests-dir <path>] [--output-format <format>]
  ...
  Formats          text (default), json
  Related          anvil skills · anvil agents · anvil doctor"
            .to_string(),
        LocalHelpTopic::BootstrapPlan => "Bootstrap Plan
  Usage            anvil bootstrap-plan [--output-format <format>]
  ...
  Formats          text (default), json
  Related          anvil doctor · anvil status"
            .to_string(),
...
fn print_acp_status(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let message = "ACP/Zed editor integration is not implemented in anvil-code yet. `anvil acp serve` is only a discoverability alias today; it does not launch a daemon or Zed-specific protocol endpoint. Use the normal terminal surfaces for now and track ROADMAP #76 for real ACP support.";
    match output_format {
        CliOutputFormat::Text => {
            println!(
                "ACP / Zed\n  Status           discoverability only\n  Launch           `anvil acp serve` / `anvil --acp` / `anvil -acp` report status only; no editor daemon is available yet\n  Today            use `anvil prompt`, the REPL, or `anvil doctor` for local verification\n  Tracking         ROADMAP #76\n  Message          {message}"
            );
        }
        CliOutputFormat::Json => {
            println!(
...
fn format_review_report(scope: Option<&str>) -> String {
    format!(
        "Review
  Scope            {}
  Action           review current workspace changes for bugs, security issues, and code quality
  Output           structured findings with severity levels and actionable recommendations",
        scope.unwrap_or("the current repository")
    )
}
...
fn format_plan_report(task: Option<&str>) -> String {
    format!(
        "Plan
  Task             {}
  Action           create a structured implementation plan
  Output           numbered steps, files to modify, potential risks, and complexity estimate",
...
/// Build a minimal runtime plugin state for bare mode.
/// Skips plugin initialization, MCP discovery, and hook loading.
fn build_bare_runtime_plugin_state() -> RuntimePluginState {
    RuntimePluginState {
...
fn build_runtime(
    ...
    let runtime_plugin_state = if bare {
        build_bare_runtime_plugin_state()
    } else {
...
fn build_runtime_with_plugin_state(
    ...
    runtime = runtime.with_max_budget_usd(max_budget_usd);
    if let Some(turns) = max_turns {
        runtime = runtime.with_max_iterations(turns);
    }
...
fn format_context_window_blocked_error(session_id: &str, error: &api::ApiError) -> String {
    ...
    lines.push(format!(
        "  Resume compact   anvil --resume {session_id} /compact"
    ));
...
fn format_tool_call_start(name: &str, input: &str) -> String {
    ...
    format!(
        "\x1b[38;5;245m╭─ \x1b[38;5;245m{name}\x1b[0;38;5;245m ─╮\x1b[0m\n\x1b[38;5;245m│\x1b[0m {detail}\n\x1b[38;5;245m╰{border}╯\x1b[0m"
    )
}
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    ...
    writeln!(
        out,
        "  anvil --resume [SESSION.jsonl|session-id|latest] [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
        "      Inspect or maintain a saved session without entering the REPL"
    )?;
    writeln!(out, "  anvil help")?;
    writeln!(out, "      Alias for --help")?;
    writeln!(out, "  anvil version")?;
    writeln!(out, "      Alias for --version")?;
    writeln!(out, "  anvil status")?;
    writeln!(
        out,
        "      Show the current local workspace status snapshot"
    )?;
    writeln!(out, "  anvil sandbox")?;
    writeln!(out, "      Show the current sandbox isolation snapshot")?;
    writeln!(out, "  anvil doctor")?;
    writeln!(
        out,
        "      Diagnose local auth, config, workspace, and sandbox health"
    )?;
    writeln!(out, "  anvil acp [serve]")?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  --bare                     Start in minimal mode; skip plugins, MCP, hooks, and other non-essential startup"
    )?;
    writeln!(
        out,
        "  --from-pr [NUMBER]         Start session with context from a GitHub PR (opens picker if NUMBER omitted)"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  --chrome                   Enable Chrome browser integration (headless CDP)"
    )?;
    writeln!(out, "  --allowedTools TOOLS       Restrict enabled tools (repeatable; comma-separated aliases supported)")?;
    writeln!(
        out,
        "  --json-schema SCHEMA       Enforce JSON output matching the provided JSON Schema (prompt mode only)"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  REPL turns auto-save to .anvil/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  anvil --compact \"summarize Cargo.toml\" | wc -l")?;
    writeln!(
        out,
        "  anvil --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(out, "  anvil --resume {LATEST_SESSION_REFERENCE}")?;
    writeln!(
        out,
        "  anvil --resume {LATEST_SESSION_REFERENCE} /status /diff /export notes.txt"
    )?;
    writeln!(out, "  anvil agents")?;
    writeln!(out, "  anvil mcp show my-server")?;
    writeln!(out, "  anvil /skills")?;
    writeln!(out, "  anvil doctor")?;
    ...
    writeln!(out, "  anvil init")?;
    writeln!(out, "  anvil export")?;
    writeln!(out, "  anvil export conversation.md")?;
    Ok(())
}
...
mod tests {
    use super::{
        ...
        format_pr_report, format_resume_report, format_review_report, format_status_report,
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        ...
        assert!(
            rendered.contains("Resume compact   anvil --resume session-issue-32 /compact"),
            "{rendered}"
        );
        assert!(
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        std::fs::create_dir_all(cwd.join(".anvil")).expect("project config dir should exist");
    ...
    fn export_help_json_is_bounded_and_parseable_384() {
        ...
        assert_eq!(
            value["usage"],
            "anvil export [--session <id|latest>] [--output <path>] [--output-format <format>]"
        );
    ...
    fn status_degrades_gracefully_on_malformed_mcp_config_143() {
        ...
        // `anvil doctor` already degrades gracefully; this test locks `status`
...
"#,
    ...
    fn state_error_surfaces_actionable_worker_commands_139() {
        // #139: the error for missing `.anvil/worker-state.json` must name
        ...
        assert!(
            message.contains("anvil prompt"),
            "error should name `anvil prompt <text>` as a producer: {message}"
        );
        assert!(
        ...
        assert!(
            state_help.contains("anvil prompt"),
            "state help must name `anvil prompt <text>` as a producer: {state_help}"
        );
    }
    ...
    fn parses_direct_agents_mcp_and_skills_slash_commands() {
        ...
        assert!(error.contains("anvil --resume SESSION.jsonl /status"));
    }
    ...
    fn punctuation_bearing_single_token_still_dispatches_to_prompt() {
        ...
        // doesn't pick up a stale .anvil/settings.json from other tests that
    ...
    fn rejects_unknown_options_with_helpful_guidance() {
        ...
        assert!(error.contains("anvil --help"));
    }
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains("Auto-save            .anvil/sessions/<session-id>.jsonl"));
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("anvil help"));
        assert!(help.contains("anvil version"));
        assert!(help.contains("anvil status"));
        assert!(help.contains("anvil sandbox"));
        assert!(help.contains("anvil init"));
        assert!(help.contains("anvil acp [serve]"));
        assert!(help.contains("anvil agents"));
        assert!(help.contains("anvil mcp"));
        assert!(help.contains("anvil skills"));
        assert!(help.contains("anvil /skills"));
        assert!(help.contains("anthropics/anvil"));
        assert!(help.contains("cargo install anvil"));
        assert!(!help.contains("anvil login"));
        assert!(!help.contains("anvil logout"));
    }
    ...
    fn session_lifecycle_prefers_running_process_over_idle_shell() {
        ...
        let lifecycle = classify_session_lifecycle_from_panes(
            ...
            vec![
                ...
                TmuxPaneSnapshot {
                    pane_id: "%2".to_string(),
                    current_command: "anvil".to_string(),
        ...
        assert_eq!(lifecycle.pane_command.as_deref(), Some("anvil"));
    ...
    fn session_list_surfaces_saved_dirty_abandoned_lifecycle() {
        ...
        git(&["config", "user.email", "tests@example.com"], &workspace);
        git(&["config", "user.name", "Anvil Tests"], &workspace);
        fs::write(workspace.join(".gitignore"), ".anvil/\n").expect("write gitignore");
        fs::write(workspace.join("tracked.txt"), "hello\n").expect("write tracked");
    ...
    fn status_json_surfaces_session_lifecycle_for_clawhip() {
        let context = super::StatusContext {
            ...
            session_lifecycle: SessionLifecycleSummary {
                ...
                pane_command: Some("anvil".to_string()),
                pane_path: Some(PathBuf::from("/tmp/project")),
    ...
    fn runtime_slash_reports_describe_command_behavior() {
        ...
        let review = format_review_report(Some("src/main.rs"));
        assert!(review.contains("Scope            src/main.rs"));
        assert!(review.contains(
            "review current workspace changes for bugs, security issues, and code quality"
        ));
...
UU conflicted.rs",
    ...
    fn help_mentions_jsonl_resume_examples() {
        ...
        print_help_to(&mut help).expect("help should render");
        let help = String::from_utf8(help).expect("help should be utf8");
        assert!(help.contains("anvil --resume [SESSION.jsonl|session-id|latest]"));
        assert!(help.contains("Use `latest` with --resume, /resume, or /session switch"));
        assert!(help.contains("anvil --resume latest"));
        assert!(help.contains("anvil --resume latest /status /diff /export notes.txt"));
    }
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = workspace.join(".anvil/sessions/legacy.json");
    ...
    fn load_session_reference_rejects_workspace_mismatch() {
        ...
        let session_path = workspace_a.join(".anvil/sessions/legacy-cross.jsonl");
        std::fs::create_dir_all(
            session_path
                .parent()
    ...
    fn resume_usage_mentions_latest_shortcut() {
        ...
        assert!(usage.contains(".anvil/sessions/<session-id>.jsonl"));
    ...
    fn temp_workspace(label: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("anvil-cli-{label}-{nanos}"))
    }
    ...
    #[test]
    fn rejects_invalid_effort_value() {
        let err = parse_args(&[
            "--effort".to_string(),
            "turbo".to_string(),
            "prompt".to_string(),
        ...
        .unwrap_err();
        assert!(
            err.contains("invalid value for --effort"),
            "unexpected error: {err}"
        );
        assert!(err.contains("turbo"), "unexpected error: {err}");
    }
    ...
    #[test]
    fn accepts_valid_effort_values() {
        for value in ["low", "medium", "high"] {
            let result = parse_args(&[
                "--effort".to_string(),
                value.to_string(),
            ...
            assert!(
                result.is_ok(),
                "--effort {value} should be accepted, got: {result:?}"
            );
            if let Ok(CliAction::Prompt {
                reasoning_effort, ..
            }) = result
    ...
    #[test]
    fn accepts_effort_equals_syntax() {
        let result = parse_args(&[
        ...
        assert!(
            result.is_ok(),
            "--effort=high should be accepted, got: {result:?}"
        );
        if let Ok(CliAction::Prompt {
            reasoning_effort, ..
        }) = result
    ...
    fn parses_json_schema_flag() {
        ...
        assert!(
            result.is_ok(),
            "--json-schema should be accepted, got: {result:?}"
        );
        if let Ok(CliAction::Prompt { json_schema, .. }) = result {
            assert_eq!(json_schema.as_deref(), Some(schema));
        }
    }
    ...
    fn rejects_invalid_json_schema() {
        ...
        let err = parse_args(&args).expect_err("invalid JSON should be rejected");
        assert!(
            err.contains("invalid JSON for --json-schema"),
            "unexpected error: {err}"
        );
    }
    ...
    fn parses_fallback_model_flag() {
        ...
        assert!(
            result.is_ok(),
            "--fallback-model should be accepted, got: {result:?}"
        );
        if let Ok(CliAction::Prompt { fallback_model, .. }) = result {
            assert_eq!(fallback_model.as_deref(), Some("opus"));
        }
    }
    ...
    fn parses_fallback_model_equals_syntax() {
        ...
        if let Ok(CliAction::Prompt { fallback_model, .. }) = result {
            assert_eq!(fallback_model.as_deref(), Some("sonnet"));
        }
    }
    ...
    fn parses_bare_flag() {
        ...
        let result = parse_args(&args);
        assert!(result.is_ok(), "--bare should be accepted, got: {result:?}");
        if let Ok(CliAction::Prompt { bare, .. }) = result {
            assert!(bare, "bare should be true when --bare is passed");
        }
    }
    ...
    fn bare_defaults_to_false() {
        ...
        if let Ok(CliAction::Prompt { bare, .. }) = result {
            assert!(!bare, "bare should default to false");
        }
    }
    ...
    fn parses_from_pr_flag_with_number() {
        ...
        assert!(
            result.is_ok(),
            "--from-pr should be accepted, got: {result:?}"
        );
        if let Ok(CliAction::Repl { from_pr, .. }) = result {
            assert_eq!(from_pr, Some(123), "from_pr should be Some(123)");
        }
    }
    ...
    fn parses_from_pr_flag_with_equals() {
        ...
        assert!(
            result.is_ok(),
            "--from-pr=456 should be accepted, got: {result:?}"
        );
        if let Ok(CliAction::Repl { from_pr, .. }) = result {
            assert_eq!(from_pr, Some(456), "from_pr should be Some(456)");
        }
    }
    ...
    fn parses_from_pr_flag_without_number() {
        ...
        assert!(
            result.is_ok(),
            "--from-pr without number should be accepted, got: {result:?}"
        );
        if let Ok(CliAction::Repl { from_pr, .. }) = result {
            assert_eq!(
                from_pr,
                Some(0),
                "from_pr should be Some(0) indicating picker mode"
            );
        }
    }
    ...
    fn from_pr_defaults_to_none() {
        ...
        assert!(
            result.is_ok(),
            "empty args should be accepted, got: {result:?}"
        );
        if let Ok(CliAction::Repl { from_pr, .. }) = result {
            assert_eq!(from_pr, None, "from_pr should default to None");
        }
    }

    #[test]
    fn rejects_from_pr_with_zero() {
        let args = vec!["--from-pr".to_string(), "0".to_string()];
        let error = parse_args(&args).expect_err("--from-pr 0 should be rejected");
        assert!(error.contains("--from-pr must be a positive integer"));
    }

    #[test]
    fn rejects_from_pr_with_invalid_value() {
        let args = vec!["--from-pr".to_string(), "abc".to_string()];
        let error = parse_args(&args).expect_err("--from-pr abc should be rejected");
        assert!(error.contains("invalid value for --from-pr"));
    }

    #[test]
    fn parses_auth_subcommand_actions() {
        // `claw auth status` (default when no action specified)
        assert_eq!(
        ...
        // `anvil auth login`
        assert_eq!(
            parse_args(&["auth".to_string(), "login".to_string()])
        ...
        // `anvil auth logout`
        assert_eq!(
            parse_args(&["auth".to_string(), "logout".to_string()])
        ...
        // `claw auth status --output-format json`
        assert_eq!(
            parse_args(&[
                "auth".to_string(),
                "status".to_string(),
    ...
    #[test]
    fn auth_rejects_unknown_actions() {
        let error = parse_args(&["auth".to_string(), "invalid".to_string()])
            .expect_err("unknown auth action should fail");
        assert!(
            error.contains("unknown auth action"),
            "error should mention unknown action: {error}"
        );
        assert!(
            error.contains("invalid"),
            "error should name the invalid action: {error}"
        );
    }

    #[test]
    fn auth_rejects_extra_arguments() {
        let error = parse_args(&["auth".to_string(), "login".to_string(), "extra".to_string()])
            .expect_err("extra args should fail");
        assert!(
            error.contains("unexpected extra arguments"),
            "error should mention extra args: {error}"
        );
    }

    #[test]
    fn auth_status_shows_logged_out_when_no_token() {
        let _guard = env_lock();
        let config_home = std::env::temp_dir().join(format!(
            "anvil_test_auth_status_no_token_{}_{}",
            std::process::id(),
        ...
        // When no credentials exist, status should show not logged in
        let result = super::run_auth_status(CliOutputFormat::Text);
    ...
    fn auth_logout_clears_token() {
        ...
        runtime::save_oauth_credentials(&token_set).expect("save credentials");
        ...
        // Logout should clear the credentials
        let result = super::run_auth_logout(CliOutputFormat::Text);
        assert!(result.is_ok(), "auth logout should succeed: {result:?}");

        // Verify credentials are cleared
        assert!(
            runtime::load_oauth_credentials()
                .expect("load credentials after logout")
                .is_none(),
            "credentials should not exist after logout"
        );
    ...
    #[test]
    fn auth_status_shows_logged_in_when_token_exists() {
        let _guard = env_lock();
        let config_home = std::env::temp_dir().join(format!(
            "anvil_test_auth_status_with_token_{}_{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        let token_set = runtime::OAuthTokenSet {
            access_token: "test-access-token".to_string(),
            refresh_token: None,
            expires_at: Some(9999999999), // Far future timestamp
            scopes: vec!["org:read".to_string()],
        };
        runtime::save_oauth_credentials(&token_set).expect("save credentials");

        // Status should show logged in
        let result = super::run_auth_status(CliOutputFormat::Text);
        assert!(result.is_ok(), "auth status should succeed: {result:?}");
...
mod dump_manifests_tests {
    ...
    fn dump_manifests_uses_explicit_manifest_dir() {
        let root = std::env::temp_dir().join(format!(
            "anvil_test_explicit_manifest_dir_{}",
            std::process::id()
        ));
        let workspace = root.join("workspace");
