...
pub const DEFAULT_DEEPSEEK_OPENROUTER_URL: &str = "https://openrouter.ai/api/v1";
pub const DEFAULT_DEEPSEEK_TOGETHER_URL: &str = "https://api.together.xyz/v1";
...
impl OpenAiCompatConfig {
    ...
    /// DeepSeek endpoint via OpenAI-compatible protocol.
    /// Uses the OpenAI-compatible REST shape at api.deepseek.com/v1/chat/completions.
    #[must_use]
    ...
    /// DeepSeek via OpenRouter (openrouter.ai).
    /// Routes DeepSeek models through OpenRouter's API gateway.
    #[must_use]
    pub const fn deepseek_openrouter() -> Self {
        Self {
            provider_name: "OpenRouter",
            api_key_env: "OPENROUTER_API_KEY",
            base_url_env: "OPENROUTER_BASE_URL",
            default_base_url: DEFAULT_DEEPSEEK_OPENROUTER_URL,
            max_request_body_bytes: OPENAI_MAX_REQUEST_BODY_BYTES,
        }
    }

    /// DeepSeek via Together.ai (api.together.xyz).
    /// Routes DeepSeek models through Together's API gateway.
    #[must_use]
    pub const fn deepseek_together() -> Self {
        Self {
            provider_name: "Together",
            api_key_env: "TOGETHER_API_KEY",
            base_url_env: "TOGETHER_BASE_URL",
            default_base_url: DEFAULT_DEEPSEEK_TOGETHER_URL,
            max_request_body_bytes: OPENAI_MAX_REQUEST_BODY_BYTES,
        }
    }
...
fn strip_routing_prefix(model: &str) -> &str {
    if let Some(pos) = model.find('/') {
        ...
        if matches!(
            prefix,
            "openai" | "xai" | "grok" | "qwen" | "kimi" | "deepseek" | "ds" | "openrouter" | "together"
        ) {
            &model[pos + 1..]
        } else {
            model
...
pub fn build_chat_completion_request(
    ...
    // When enabled, sends `thinking: {"type": "enabled"}`.
    // When disabled, sends `thinking: {"type": "disabled"}`.
    // Omitted when None so the backend uses its default.
    if let Some(thinking) = &request.thinking {
        payload["thinking"] = json!({
            "type": thinking.mode,
        });
    }
    // DeepSeek reasoning_effort can be set independently of thinking.
...
pub fn translate_message(message: &InputMessage, model: &str) -> Vec<Value> {
    ...
    match message.role.as_str() {
        "assistant" => {
            ...
            // DeepSeek V4 Pro requires reasoning_content on every assistant
            // message in the history when thinking mode is enabled, even when
            // the message only contains tool_calls. If reasoning is empty but
            // the model requires it, include an empty string.
            let force_reasoning = model_requires_reasoning_content_in_history(model)
                && reasoning.is_empty()
                && !tool_calls.is_empty();
            if text.is_empty() && tool_calls.is_empty() && !include_reasoning && !force_reasoning {
                Vec::new()
            } else {
...
mod tests {
    ...
    fn model_rejects_is_error_field_detects_kimi_models() {
        ...
        assert!(!super::model_rejects_is_error_field("deepseek-v4-pro"));
    ...
    fn translate_message_includes_is_error_for_non_kimi_models() {
        ...
        // Test with model (should include is_error)
        let translated3 = super::translate_message(&message, "deepseek-v4-pro");
        assert!(
            translated3[0].get("is_error").is_some(),
            "response should include is_error field"
        );
    }
