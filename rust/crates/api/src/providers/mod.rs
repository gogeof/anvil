...
const MODEL_REGISTRY: &[(&str, ProviderMetadata)] = &[
    ...
    (
        ...
        ProviderMetadata {
            ...
            default_base_url: openai_compat::DEFAULT_DEEPSEEK_OPENROUTER_URL,
        },
    ),
    ...
    (
        ...
        ProviderMetadata {
            ...
            default_base_url: openai_compat::DEFAULT_DEEPSEEK_TOGETHER_URL,
        },
    ),
];
    ...
    MODEL_REGISTRY
        ...
        .find_map(|(alias, metadata)| {
            (*alias == lower).then_some(match metadata.provider {
                ...
                ProviderKind::OpenRouter | ProviderKind::Together => trimmed,
            })
        })
...
pub fn metadata_for_model(model: &str) -> Option<ProviderMetadata> {
    ...
    // DeepSeek models — routes deepseek/* and deepseek-* and ds-*
    // model names through the OpenAI-compatible client pointed at
    // api.deepseek.com.
    if canonical.starts_with("deepseek/")
        || canonical.starts_with("deepseek-")
        || canonical.starts_with("ds/")
        || canonical.starts_with("ds-")
    {
        return Some(ProviderMetadata {
            provider: ProviderKind::DeepSeek,
    ...
    // OpenRouter gateway — routes openrouter/* model names through
    // OpenRouter's API gateway (openrouter.ai).
    ...
    // Together.ai gateway — routes together/* model names through
    // Together's API gateway (api.together.xyz).
...
pub const fn model_family_identity_for_kind(kind: ProviderKind) -> runtime::ModelFamilyIdentity {
    match kind {
        ...
        | ProviderKind::Together => runtime::ModelFamilyIdentity::Generic,
    }
}
...
const FOREIGN_PROVIDER_ENV_VARS: &[(&str, &str, &str)] = &[
    ...
    (
        "DEEPSEEK_API_KEY",
        "DeepSeek",
        "prefix your model name with `deepseek/` or `ds-` (e.g. `--model ds-pro`) so prefix routing selects the DeepSeek backend",
    ),
];
...
mod tests {
    ...
    fn plugin_config_max_output_tokens_overrides_model_default() {
        ...
        std::fs::create_dir_all(cwd.join(".anvil")).expect("project config dir");
