...
pub struct MessageRequest {
    ...
    /// When `Some(ThinkingConfig { enabled: true })`, sends `thinking: {"type": "enabled"}`.
    /// When `Some(ThinkingConfig { enabled: false })`, sends `thinking: {"type": "disabled"}`.
...
/// Serializes as `{"type": "enabled"}` or `{"type": "disabled"}`.
