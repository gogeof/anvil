...
fn detect_unsupported_manifest_contracts(
    ...
    for (field, detail) in [
        (
            "skills",
            "plugin manifest field 'skills' uses an unsupported contract. anvil discovers skills from .anvil/skills, .agents/skills, and ~/.anvil/skills.",
        ),
        (
            "mcpServers",
            "plugin manifest field 'mcpServers' is not supported by anvil. Configure MCP servers via settings.json.",
        ),
        (
            "agents",
            "plugin manifest field 'agents' is not supported by anvil.",
        ),
    ] {
    ...
    {
        errors.push(PluginManifestValidationError::UnsupportedManifestContract {
            detail: "plugin manifest field 'commands' uses directory glob patterns not supported by anvil. anvil slash commands are built-in only.".to_string(),
        });
    }
    ...
    if let Some(hooks) = root.get("hooks").and_then(Value::as_object) {
        for hook_name in hooks.keys() {
            if !matches!(
                ...
                errors.push(PluginManifestValidationError::UnsupportedManifestContract {
                    detail: format!(
                        "plugin hook '{hook_name}' is not supported. anvil plugins support PreToolUse, PostToolUse, and PostToolUseFailure only."
                    ),
                });
            }
        }
...
/// Guards against concurrent modification of `ANVIL_CONFIG_HOME`.
#[cfg(test)]
...
mod tests {
  ...
  "description": "Packaged manifest in install root"
    ...
    /// Regression test for ROADMAP #41: verify that `ANVIL_CONFIG_HOME` isolation prevents
    /// host `~/.anvil/plugins/` from bleeding into test runs.
    #[test]
    fn claw_config_home_isolation_prevents_host_plugin_leakage() {
        let _guard = env_guard();

        // Create a temp directory to act as our isolated ANVIL_CONFIG_HOME
        let config_home = temp_dir("isolated-home");
        let bundled_root = temp_dir("isolated-bundled");

        // Set ANVIL_CONFIG_HOME to our temp directory
  ...
  "description": "Test fixture plugin in isolated config home"
        ...
        // Create PluginManager with isolated bundled_root - it should use the temp config_home, not host ~/.anvil/
        let mut config = PluginManagerConfig::new(&config_home);
        config.bundled_root = Some(bundled_root.clone());
        let manager = PluginManager::new(config);
        ...
        assert_eq!(
            installed.len(),
            1,
            "should only see the test fixture plugin, not host ~/.anvil/plugins/"
        );
        assert_eq!(
