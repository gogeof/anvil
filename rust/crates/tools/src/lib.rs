...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "Browser",
            description: "Control a headless Chrome/Chromium browser via Chrome DevTools Protocol (CDP). Actions: navigate, screenshot, get_content, evaluate, click, type, close.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "action": {
                        "type": "string",
                        "enum": ["navigate", "screenshot", "get_content", "evaluate", "click", "type", "close"]
                    },
                    ...
                    "script": { "type": "string", "description": "JavaScript to evaluate (required for evaluate action)" },
...
fn skill_lookup_roots() -> Vec<SkillLookupRoot> {
    ...
    if let Ok(claw_config_home) = std::env::var("ANVIL_CONFIG_HOME") {
    ...
    push_skill_lookup_root(
        &mut roots,
        std::path::PathBuf::from("/home/bellman/.anvil/skills"),
        SkillLookupOrigin::SkillsDir,
    );
    push_skill_lookup_root(
        &mut roots,
...
impl ProviderRuntimeClient {
    ...
    fn new_with_fallback_config(
        ...
        let store_model = primary_model.clone();
        ...
        .map_err(|_| {
            String::from(
                "HOME is not set (on Windows, set USERPROFILE or HOME, \
                 or use ANVIL_CONFIG_HOME to point directly at the config directory)",
            )
        })?;
...
fn run_browser(input: BrowserInput) -> Result<String, String> {
    #[cfg(not(feature = "chrome"))]
    {
        let _ = input;
        Err(
            "Browser tool requires 'chrome' feature to be enabled. Rebuild with --features chrome"
                .to_string(),
        )
    }
        ...
        match input.action {
            BrowserAction::Navigate => {
                let url = input
                    .url
                    .as_ref()
                    .ok_or_else(|| "url is required for navigate action".to_string())?;

                // Get or create browser session
                let session_guard = global_browser_session()
                    .lock()
                    .map_err(|e| format!("Failed to acquire browser lock: {e}"))?;
                ...
                let session = session.as_ref().ok_or("No browser session available")?;
                ...
                if let Some(ref selector) = input.wait_for {
                    session
                        .tab
            ...
            BrowserAction::Screenshot => {
                ...
                let session = session_guard.as_ref().ok_or_else(|| {
                    "No active browser session. Use navigate action first.".to_string()
                })?;
                ...
                let encoded = base64_encode(&screenshot_data);
            ...
            BrowserAction::GetContent => {
                ...
                let remote_obj = session
                    .tab
                    .evaluate("document.body ? document.body.innerText : ''", true)
                    .map_err(|e| format!("Failed to get page content: {e}"))?;

                let content = remote_obj
                    .value
            ...
            BrowserAction::Evaluate => {
                let script = input
                    .script
                    .as_ref()
                    .ok_or_else(|| "script is required for evaluate action".to_string())?;
                ...
                let result = remote_obj.value.clone().unwrap_or(Value::Null);
            ...
            BrowserAction::Click => {
                let selector = input
                    .selector
                    .as_ref()
                    .ok_or_else(|| "selector is required for click action".to_string())?;
                ...
                // Small delay for page to respond
            ...
            BrowserAction::Type => {
                let selector = input
                    .selector
                    .as_ref()
                    .ok_or_else(|| "selector is required for type action".to_string())?;
                let text = input
                    .text
                    .as_ref()
            ...
            BrowserAction::Close => {
                let mut session_guard = global_browser_session()
                    .lock()
                    .map_err(|e| format!("Failed to acquire browser lock: {e}"))?;
...
mod tests {
    ...
    fn recovery_loop_state_file_reflects_transitions() {
        // End-to-end proof: .anvil/worker-state.json reflects every transition
        ...
        let state_path = worktree.join(".anvil").join("worker-state.json");
    ...
    fn web_search_extracts_and_filters_results() {
        ...
        // sets ANVIL_WEB_SEARCH_BASE_URL. Without the lock, parallel test
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".anvil").join("skills").join("plan");
        let command_dir = root.join(".anvil").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(skill_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".anvil/skills/plan/SKILL.md"));
        ...
        assert!(command_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".anvil/commands/handoff.md"));
  ...
  "nbformat_minor": 5
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        let local_settings = std::fs::read_to_string(cwd.join(".anvil").join("settings.local.json"))
            .expect("local settings after enter");
        assert!(local_settings.contains(r#""defaultMode": "plan""#));
        let state =
            std::fs::read_to_string(cwd.join(".anvil").join("tool-state").join("plan-mode.json"))
